...
- `description`은 **한글 75~90자**, 글마다 고유하게. (구글 검색결과 설명은 약 920px에서 잘리는데 한글은 글자폭이 영문의 약 2배라 75~90자가 표시 한계. 영문 기준 120~155자를 한글에 그대로 적용하면 뒤가 잘림.) `<title>`은 `{제목} — 주재원노트` 자동 생성.
...
- **내부 링크는 확장자 없는 clean URL로**: `[제목](/settle/bank-account)` 형식(`.html` 붙이지 말 것). Cloudflare가 확장자 없는 주소로 서빙하며, `.html`을 쓰면 307 리다이렉트가 생겨 canonical과 불일치함. 빌드가 만드는 링크·canonical·sitemap도 전부 확장자 없음.
